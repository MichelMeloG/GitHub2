...
class Pocao(Personagem):
    ...
    def resultado(self, aventureiro):
        impacto = random.choices(["Dobrar a vida", "Aumentar a força", "Aumenta a defesa"], [0.2, 0.4, 0.4])
        match impacto:
            case "Dobrar a vida":
                aventureiro.status = "Tomou uma poção que dobrou sua vida!"
                aventureiro.vida *= 2
                aventureiro.vida_inicial *= 2
            case "Aumentar a força":
                aventureiro.status = "Tomou uma poção que aumentou em 15 sua força!"
                aventureiro.forca += 15
            case "Aumentar a defesa":
